...
contract. `workstation.html` is the integration surface. The full compliance
package for the Phase 1 build lives in `compliance/` — open
`compliance/INDEX.html` for the portal, or `compliance/README.md` for the
text index. Remaining follow-on work: (1) firm-side CCO sign-off of the
regulated templates via `compliance/11-sign-off-template.md`, (2) wiring the
market-data MCP into Modules 3 and 8, (3) deeper embedding into the legacy
`ae_terminal.html`.
...
> reviewed and owned by a qualified compliance professional at the
> implementing firm before any use on real client files. The compliance
> package (`compliance/`) is the dossier they review. Module 7 is
> affirmative and non-blocking by design.
